        ...
        $('#bidForm').submit(function(event) {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error(error);
                    alert('An error occurred while trying again.');
                }
            });
            $.ajax({
                type: 'GET',
                ...
                success: function(resp) {
                    var start = resp.indexOf('<table id="bidTable">');
                    var end = resp.indexOf('</table>');
                    var str = resp.substring(start, end + 8);
                    $('#bidTable').html(str);
                },
                error: function(xhr, status, error) {
                    console.error(error);
                    alert('Failed to fetch updated auction details.');
                }
            });
        });
    });
